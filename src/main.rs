mod error;
mod commands;

use crate::error::CatBoatError;
use std::{
	env,
	sync::Arc,
	collections::HashSet
};
use anyhow::{Error, Context as AnyhowContext};
use hyper_tls::HttpsConnector;
use serenity::{
	async_trait,
	client::bridge::gateway::ShardManager,
	framework::{
		StandardFramework,
		standard::macros::group,
	},
	http::Http,
	model::{event::ResumedEvent, gateway::Ready},
	prelude::*,
};
use tracing::{error, info};
use tracing_subscriber::{
	FmtSubscriber,
	EnvFilter,
};
use human_panic::setup_panic;

use commands::{
	debug::*,
	owner::*,
	prnhb::*
};

pub type StandardResult<T> = Result<T, Error>;

struct ShardManagerContainer;

impl TypeMapKey for ShardManagerContainer {
	type Value = Arc<Mutex<ShardManager>>;
}

struct Handler;

#[async_trait]
impl EventHandler for Handler {
	async fn ready(&self, _: Context, ready: Ready) {
		info!("Connected as {}", ready.user.name);
	}

	async fn resume(&self, _: Context, _: ResumedEvent) {
		info!("Resumed");
	}
}

use lazy_static::lazy_static;

lazy_static! {
	static ref HTTPS_CLIENT: hyper::Client<HttpsConnector<hyper::client::HttpConnector>> = hyper::Client::builder().build::<_, hyper::Body>(HttpsConnector::new());
}

#[group]
#[commands(ping, quit, search_p)]
struct General;

#[tokio::main]
async fn main() -> StandardResult<()> {
	setup_panic!();

	dotenv::dotenv().context("Failed to load .env file")?;

	let subscriber = FmtSubscriber::builder()
		.with_env_filter(EnvFilter::from_default_env())
		.finish();
	tracing::subscriber::set_global_default(subscriber)
		.context("Failed to start the logger")
		.map_err(|_| CatBoatError::Unknown)?;

	// let client = hyper::Client::builder().build::<_, hyper::Body>(HttpsConnector::new());
	let token = env::var("DISCORD_TOKEN")
		.context("Expected a token in the environment").map_err(|_| CatBoatError::Unknown)?;

	let http = Http::new_with_token(&token);

	// We will fetch your bot's owners and id
	let (owners, _bot_id) = match http.get_current_application_info().await {
		Ok(info) => {
			let mut owners = HashSet::new();
			owners.insert(info.owner.id);

			(owners, info.id)
		},
		Err(why) => panic!("Could not access application info: {:?}", why),
	};

	// Create the framework
	let framework = StandardFramework::new()
		.configure(|c| c
			.owners(owners)
			.prefix("~"))
		.group(&GENERAL_GROUP);

	let mut client = Client::builder(&token)
		.framework(framework)
		.event_handler(Handler)
		.await
		.context("Err creating client")
		.map_err(|_| CatBoatError::Unknown)?;

	{
		let mut data = client.data.write().await;
		data.insert::<ShardManagerContainer>(client.shard_manager.clone());
	}

	let shard_manager = client.shard_manager.clone();

	tokio::spawn(async move {
		tokio::signal::ctrl_c().await.expect("Could not register ctrl+c handler");
		shard_manager.lock().await.shutdown_all().await;
	});

	if let Err(why) = client.start().await {
		error!("Client error: {:?}", why);
	}

	Ok(())
}
